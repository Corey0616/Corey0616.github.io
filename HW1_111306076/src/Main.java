...
public class Main {
	...
	public static double getBMI(String[] data) {
		...
		double height = Double.parseDouble(data[1])*0.01;
	...
	public static String getDiagnosis(double bmi) {
		...
		if(bmi >= 30) {
			dia = "You are not in shape. Actually, you are not even close.";
		}
		else if(bmi < 30 && bmi >= 26) {
			dia = "To be honest, you are not in shape.";
		}
		else if(bmi < 26 && bmi >= 20) {
			dia = "You are in shape.";
		}
		else {
			dia = "You are under shape.";
		}
